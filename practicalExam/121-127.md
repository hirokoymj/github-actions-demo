...
- [Practice Exam 121-127](#practice-exam-121-127)
  - [Q121: What’s true about default variables? (choose three)](#q121-whats-true-about-default-variables-choose-three) - [Default variables](#default-variables)
  - [Q122: What are the scopes defined for custom variables in a workflow? (choose three)](#q122-what-are-the-scopes-defined-for-custom-variables-in-a-workflow-choose-three) - [Custom variables](#custom-variables) - [Environment variable](#environment-variable) - [Summary](#summary)
  - [Q123: What must be added to actions/checkout if my-org/my-private-repo is a private repository differing from the one containing the current workflow?](#q123-what-must-be-added-to-actionscheckout-if-my-orgmy-private-repo-is-a-private-repository-differing-from-the-one-containing-the-current-workflow) - [💡 Why this is correct](#-why-this-is-correct)
  - [Q124: Given the following configuration, how many jobs will GitHub Actions run when this matrix is evaluated?](#q124-given-the-following-configuration-how-many-jobs-will-github-actions-run-when-this-matrix-is-evaluated)
  - [Q125: At what levels can environment variables be defined ? (Choose three)](#q125-at-what-levels-can-environment-variables-be-defined--choose-three) - [✅ Correct answers (Choose three):](#-correct-answers-choose-three)
  - [Q126: How should a dependent job reference the output1 value produced by a job named job1 earlier in the same workflow?](#q126-how-should-a-dependent-job-reference-the-output1-value-produced-by-a-job-named-job1-earlier-in-the-same-workflow) - [💡 Why this is correct](#-why-this-is-correct-1) - [🧩 Example](#-example)
  - [Q127: Which workflow command syntax correctly sets an environment variable named 'API_VERSION' with the value '2.1' for subsequent steps in a GitHub Actions job?](#q127-which-workflow-command-syntax-correctly-sets-an-environment-variable-named-api_version-with-the-value-21-for-subsequent-steps-in-a-github-actions-job)
...
**Default variables** are automatically provided by GitHub in every workflow run — you never define them yourself. Examples:

| Default Variable  | Value example            |
| ----------------- | ------------------------ |
...
**Why the 3 correct answers are true:**

- ✅ **"Accessed using the `env` context"** — you can use `${{ env.GITHUB_SHA }}` or just `$GITHUB_SHA`
- ✅ **"Most have a corresponding context property"** — e.g., `GITHUB_SHA` ↔ `${{ github.sha }}`, `GITHUB_REF` ↔ `${{ github.ref }}`
- ✅ **"Set by GitHub, not defined in a workflow"** — they exist automatically in every run

| Term                              | Example                         | Where Defined                                          | Syntax                                                          |
| --------------------------------- | ------------------------------- | ------------------------------------------------------ | --------------------------------------------------------------- |
| **GitHub Environment**            | `production`, `staging`         | Settings UI → Environments                             | `environment: production` in workflow                           |
| **Custom variable** (`vars`)      | `API_URL` inside `production`   | Settings UI → Environments → production → Add variable | `${{ vars.API_URL }}`                                           |
| **Environment variable** (`env:`) | `MY_VAR` set via `env:` in YAML | Workflow YAML file                                     | `${{ env.MY_VAR }}` or `$MY_VAR`                                |
| **Default variable**              | `GITHUB_SHA`, `CI`              | Set automatically by GitHub                            | `${{ env.GITHUB_SHA }}` or `$GITHUB_SHA` or `${{ github.sha }}` |
...
- For custom variables, the three scopes:
...
The term "environment variable" means something different in GitHub Actions — it refers to variables set with env: in the workflow YAML file:

```yaml
env:
  MY_VAR: 'hello' # ← this is an "environment variable"
```
  ...
  deploy-staging:
    ...
    steps:
      - name: Print staging API URL
        run: echo ${{ vars.API_URL }} # prints https://api.staging.com
```

![](./images/UI-environment-variables.png)
...
✅ Correct answer:👉 echo "API_VERSION=2.1" >> "$GITHUB_ENV"
