...
- [Practice Test - 21-30](#practice-test---21-30)
  ...
  - [Q22: A workflow that has only workflow_dispatch event trigger can be triggered using GitHub's REST API](#q22-a-workflow-that-has-only-workflow_dispatch-event-trigger-can-be-triggered-using-githubs-rest-api)
...
- workflow_call ✅ - reusable WF
- defaults.run: `shell`, `working-directory` ONLY, WF level, Job level
- concurrency: `group`, `cancel-in-progress: true`
- `always()` - Runs no matter what — success, failure, or cancelled.
