...
- [Practice Test - 21-30](#practice-test---21-30)
	- [Q21: What are the possible types of an input variable for a manually triggered workflow? (Select five.)](#q21-what-are-the-possible-types-of-an-input-variable-for-a-manually-triggered-workflow-select-five)
	- [Q22: A workflow that has only workflow\_dispatch event trigger can be triggered using GitHub's REST API](#q22-a-workflow-that-has-only-workflow_dispatch-event-trigger-can-be-triggered-using-githubs-rest-api)
	- [Q23: To stop a workflow from running temporarily without modifying the source code you should](#q23-to-stop-a-workflow-from-running-temporarily-without-modifying-the-source-code-you-should)
	- [Q24: What are activity types of an event used for ?](#q24-what-are-activity-types-of-an-event-used-for-)
	- [Q25: You want to create a reusable workflow CI that runs some quality checks, linting and tests on code changes. What event trigger should the CI workflow define to allow reusing it in other workflows?](#q25-you-want-to-create-a-reusable-workflow-ci-that-runs-some-quality-checks-linting-and-tests-on-code-changes-what-event-trigger-should-the-ci-workflow-define-to-allow-reusing-it-in-other-workflows)
	- [Q26: A reusable workflow named build creates zip file artifacts. How do you pass the zip file location to the caller workflow that is calling the build workflow? (Select three.)](#q26-a-reusable-workflow-named-build-creates-zip-file-artifacts-how-do-you-pass-the-zip-file-location-to-the-caller-workflow-that-is-calling-the-build-workflow-select-three)
	- [Q27: What are the valid use cases for using defaults? (Select two.)](#q27-what-are-the-valid-use-cases-for-using-defaults-select-two)
	- [Q28: How can you ensure that a workflow called Deploy Prod is always running at most one at a time?](#q28-how-can-you-ensure-that-a-workflow-called-deploy-prod-is-always-running-at-most-one-at-a-time)
	- [Q29: Your Pull Request analysis workflow uses multiple code analysis tools and takes about 20minutes to fully complete. It is triggered on pull\_request event with branches filter set to master. Therefore if a developer pushes multiple commits within few minutes multiple workflows are running in parallel. How can you stop all previous workflow runs and only run the one with latest changes?](#q29-your-pull-request-analysis-workflow-uses-multiple-code-analysis-tools-and-takes-about-20minutes-to-fully-complete-it-is-triggered-on-pull_request-event-with-branches-filter-set-to-master-therefore-if-a-developer-pushes-multiple-commits-within-few-minutes-multiple-workflows-are-running-in-parallel-how-can-you-stop-all-previous-workflow-runs-and-only-run-the-one-with-latest-changes)
	- [Q30: When will job3 run?](#q30-when-will-job3-run)
...
#3 - Use concurrency with cancel-in-progress ✅
concurrency:
