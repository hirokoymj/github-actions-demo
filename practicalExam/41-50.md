...
```yaml
# 1. Repository-level secret
${{ secrets.MY_REPO_SECRET }}

# 2. Organization-level secret (set in org settings, available to selected repos)
${{ secrets.MY_ORG_SECRET }}

# 3. Environment-level secret (scoped to a specific environment in a repo)
jobs:
  deploy:
